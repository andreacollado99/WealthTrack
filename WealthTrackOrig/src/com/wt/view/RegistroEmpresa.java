...
public class RegistroEmpresa extends JFrame {
	...
	public static final String ACT_COM_BTN_REGISTRAR_EMP = "Registrar Empresa";
	...
	private void initComponents() {
		...
		btnRegistrarme = new JButton(ACT_COM_BTN_REGISTRAR_EMP);
	...
	public Empresa obtenerDatos() {
		...
		if (nombre.isEmpty() || apellido.isEmpty() || nif.isEmpty() || email.isEmpty() || pwd.isEmpty()
				|| fechaNac.isEmpty() || tel.isEmpty()) {
			mostrarMensajeError("Todos los campos son obligatorios");
		} else {

			if (nif.length() != MAX_CARAC) {
				mostrarMensajeError("\nEl DNI debe tener 9 caracteres");
			} else if (tel.length() != MAX_CARAC) {
				mostrarMensajeError("\nEl telefono debe tener 9 caracteres");
			} else {
				empresa = new Empresa(id, nombre, apellido, nif, email, pwd, fechaNac, tel);
			}
		}
